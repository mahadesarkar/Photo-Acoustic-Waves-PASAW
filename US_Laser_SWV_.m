...
medium.sound_speed_compression = 1500 * ones(Nx, Ny);   % 9138[m/s]
medium.sound_speed_shear       = 0 * ones(Nx, Ny);   % 4675 [m/s]
...
for initialp_no = 1:press_length
    source.sxx(initialp_no,:) = 10*press(initialp_no,2:end);     % Value scalled to 5 times for better resolution
end
%}
...
sensors = round(0.3*Ny):round(10e-3/dx):round(0.3*Ny)+2*round(10e-3/dx);
